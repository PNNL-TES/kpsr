...
#### KAIZAD -- using file, don't download from GS ----
printlog("Getting list of Picarro files...")
folder_id <- as_id()
filelist <- drive_ls("~/Test")
...
## Kaizad: created new key for trial samples. 5.3.2019 ----
vmdata <- read_csv("trial_valvemap.csv", na = c("", "NA", "n/a", "na"))
keydata <- read_csv("trial_sampleID.csv", na = c("", "NA", "n/a", "na"))

printlog("Downloading", sheet)

save_data(vmdata)#, fn = VALVEMAP_FILE, scriptfolder = TRUE)
save_data(keydata)#, fn = KEY_FILE, scriptfolder = TRUE)
